...
function Game() {
  ...
  const play = (selectedCard: number[]) => {
    if (!engine || !engine.state.currentPlayerId) return;
  ...
  const duplicateSelectedCard = () => {
    ...
    const cardToDuplicate = engine.state.players[0].hand.find((card) => card.id === selectedCardId);
      ...
      <>
        ...
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              ...
              const foundCard = [
                ...allCards,
                ...engine.state.players.flatMap((player) => player.hand),
              ].find(
                (card) =>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div className="cardContainer">
            {
              // pair.map((el, index) => {
              engine.state.players[1].hand.map((el: { id: number; name?: string }, index: number) => {
          ...
          <div style={{ display: seeDiscardPile ? "none" : "grid" }}>
            <div>
              <div className="cardContainer pile">
                {
              // pair.map((el, index) => {
              engine.state.discardPile.map((el, index) => {
                const id_string = el;
                // console.log(id_string);
